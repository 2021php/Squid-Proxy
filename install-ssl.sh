...
if cat /etc/os-release | grep PRETTY_NAME | grep "Ubuntu 20.04"; then
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid/squid.conf https://raw.githubusercontent.com/2021php/Squid-Proxy/main/squid.conf
    if [ -f /sbin/iptables ]; then
        /sbin/iptables -I INPUT -p tcp --dport 8751 -j ACCEPT
        /sbin/iptables-save
    fi
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid3/squid.conf https://raw.githubusercontent.com/2021php/Squid-Proxy/main/squid.conf
    /sbin/iptables -I INPUT -p tcp --dport 8751 -j ACCEPT
    /sbin/iptables-save
    ...
    /bin/rm -f /etc/squid/squid.conf
    /usr/bin/touch /etc/squid/blacklist.acl
    /usr/bin/wget --no-check-certificate -O /etc/squid/squid.conf https://raw.githubusercontent.com/2021php/Squid-Proxy/main/squid-centos7.conf
    systemctl enable squid
    systemctl restart squid
    firewall-cmd --zone=public --permanent --add-port=8751/tcp
    firewall-cmd --reload
